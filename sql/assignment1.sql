...
select count(*) as ProductsForSale
from Inventory
...
from orderdetails od
join products p on od.productid = p.productid
where p.category = 'Computers & Accessories';
...
select top 1 c.firstname, c.lastname, count(o.orderid) as number_of_orders
from customers c
join orders o on c.customerid = o.customerid
group by c.FirstName, c.LastName
...
select top 1 p.category, sum(od.Quantity)
from Products p
...
select top 1 c.firstname, c.lastname, sum(od.quantity * p.price) as total_spending
from customers c
join orders o on c.customerid = o.customerid
join orderdetails od on o.orderid = od.orderid
join products p on od.productid = p.productid
...
select (select sum(totalamount) from orders) / 
       (select count(orderid) from orders) as average_order_value;

--10.  Write an SQL query to find the total number of orders placed by each customer and list their 
-- names along with the order count.
select c.firstname, c.lastname, 
(select count(orderid) from orders o where o.customerid = c.customerid) as total_orders
from customers c;
